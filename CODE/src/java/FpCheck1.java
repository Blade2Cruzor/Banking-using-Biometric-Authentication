 ...
 * @author sluser
 */
@WebServlet(name = "FpCheck1", urlPatterns = {"/FpCheck1"})
public class FpCheck1 extends HttpServlet {
...
rst.close();
  ...
  System.out.println(blnResult);
       ...
       String qry = "select * from finger1 where cardno='"+cn+"'";
...
session.setAttribute("score1", String.valueOf(x));

if(x==100)
       {
       response.sendRedirect("Fpauthentication_2.jsp");
           //response.sendRedirect("mail?cn="+cn+"");
       }
else if(x<10)
       {
          out.println("success"+x);
            response.sendRedirect("Fpauthentication_1.jsp?cn="+cn+"&score1="+x+"&score="+score);
       }
       else
       {    
       response.sendRedirect("Fpauthentication_2.jsp?cn="+cn+"&score1="+x+"&score="+score);
